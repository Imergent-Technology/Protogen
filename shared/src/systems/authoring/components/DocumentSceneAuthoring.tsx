...
import { Button, Input, Label, Textarea, Card, Badge, Tabs, TabsList, TabsTrigger, TabsContent } from '../../../components';
...
const DocumentSceneAuthoring: React.FC<DocumentSceneAuthoringProps> = ({
  ...
  const {
    ...
    previewScene,
    canPerformAction
  } = useSceneAuthoring<DocumentSceneData>(scene);
  ...
  // Check permissions
  const canUseDocumentAuthoring = effectivePermissions.canUseDocumentAuthoring();
  const canSave = canPerformAction('save', 'document');
  const canPreview = canPerformAction('preview', 'document');

  // Initialize HTML content
  useEffect(() => {
    if (formData?.content.html) {
      setHtmlContent(formData.content.html);
    }
  }, [formData?.content.html]);

  // Handle form field changes
  const handleFieldChange = useCallback((field: keyof DocumentSceneData, value: any) => {
    updateScene({ [field]: value } as Partial<DocumentSceneData>);
  }, [updateScene]);
  ...
  const handleMetadataChange = useCallback((field: string, value: any) => {
    ...
    } as Partial<DocumentSceneData>);
  }, [updateScene, formData?.metadata]);

  const handleConfigChange = useCallback((field: keyof DocumentSceneData['config'], value: any) => {
    updateScene({
      config: {
        ...formData?.config,
        [field]: value
  ...
  }, [updateScene, formData?.config]);

  const handleStyleChange = useCallback((field: keyof DocumentSceneData['style'], value: any) => {
    updateScene({
      style: {
        ...formData?.style,
        [field]: value
  ...
  }, [updateScene, formData?.content]);

  // Handle media management
  const addMedia = useCallback((media: Omit<DocumentMedia, 'id'>) => {
    ...
    updateScene({
      content: {
        ...formData?.content,
        media: [...(formData?.content.media || []), newMedia]
      }
    } as Partial<DocumentSceneData>);
  }, [updateScene, formData?.content]);
  ...
  const removeMedia = useCallback((mediaId: string) => {
    updateScene({
      content: {
        ...formData?.content,
        media: formData?.content.media?.filter(media => media.id !== mediaId) || []
      }
    } as Partial<DocumentSceneData>);
  }, [updateScene, formData?.content]);
  ...
  const addLink = useCallback((link: Omit<DocumentLink, 'id'>) => {
    ...
    updateScene({
      content: {
        ...formData?.content,
        links: [...(formData?.content.links || []), newLink]
      }
    } as Partial<DocumentSceneData>);
  }, [updateScene, formData?.content]);
  ...
  const removeLink = useCallback((linkId: string) => {
    updateScene({
      content: {
        ...formData?.content,
        links: formData?.content.links?.filter(link => link.id !== linkId) || []
      }
    } as Partial<DocumentSceneData>);
  }, [updateScene, formData?.content]);
  ...
  }, [newTag, formData?.metadata.tags, updateScene]);
  ...
  }, [updateScene, formData?.metadata.tags]);
  ...
  if (!canUseDocumentAuthoring) {
    return (
      <Card className="p-8 text-center">
        <div className="text-4xl mb-4">🔒</div>
        <h3 className="text-lg font-semibold mb-2">Permission Required</h3>
        <p className="text-muted-foreground mb-4">
          You need Member level access to use document authoring tools.
        </p>
  ...
  return (
    <div className={`document-scene-authoring ${className}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-semibold">Document Scene Authoring</h2>
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as any)}>
        ...
        <TabsContent value="media" className="space-y-6">
          <Card className="p-6">
            ...
            {formData?.content.media && formData.content.media.length > 0 ? (
                    ...
                    <div className="space-y-1">
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          updateScene({
                            content: { ...formData.content, media: updatedMedia }
                          } as Partial<DocumentSceneData>);
                        }}
                        className="text-xs"
                      />
