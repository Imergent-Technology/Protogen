...
export const OAuthLogin: React.FC<OAuthLoginProps> = ({ 
  ...
  const handleOAuthLogin = async (provider: string) => {
    ...
    try {
      ...
      window.location.href = `http://api.protogen.local:3333/api/auth/oauth/${provider}/redirect`;
    } catch (error) {
  ...
  const handleLogout = async () => {
    ...
    try {
      const response = await fetch('http://api.protogen.local:3333/api/auth/oauth/logout', {
        method: 'POST',
        headers: {
